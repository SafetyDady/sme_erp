...
def update_item(
    ...
    # Check if new code conflicts with other items
    if payload.code != item.code:
        exists = db.scalar(select(Item).where(Item.code == payload.code, Item.id != item_id))
        if exists:
            raise HTTPException(status_code=409, detail="Item code already exists")
...
def delete_item(
    ...
    # Check if item has stock balances
    has_stock = db.scalar(select(StockBalance).where(StockBalance.item_id == item_id))
    if has_stock:
        raise HTTPException(status_code=409, detail="Cannot delete item with existing stock")
    
    db.delete(item)
    db.commit()
    return {"message": f"Item {item.code} deleted successfully"}
